...
class OPTWithClassifier(OPTForSequenceClassification):
    def __init__(self, config):
        ...
        self.temperature = 2
        self.soft_target_loss_weight = 0.2
    ...
    def forward(
        ...
        if self.teacher_model is not None:
            ...
            with torch.no_grad():
                teacher_res = self.teacher_model(
                                input_ids,
                                attention_mask,
                                head_mask,
                                past_key_values,
        ...
        if self.teacher_model is not None:
            pdb.set_trace()
            teacher_soft_targets = nn.functional.softmax(teacher_res.logits/self.temperature,dim=1)
            student_soft_prob = nn.functional.softmax(pooled_logits/self.temperature, dim=1)
            soft_targets_loss = torch.sum(teacher_soft_targets * (teacher_soft_targets.log() - student_soft_prob)) / student_soft_prob.size()[0] * (self.temperature**2)

            loss = ((1-self.soft_target_loss_weight) * loss) + (self.soft_target_loss_weight * soft_targets_loss)
...
class OPTWithLMClassifier(OPTForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        #Hack way for us to inject hardcoded teacher 
        self.teacher_model = None
        self.temperature = 2
        self.soft_target_loss_weight = 0.2
    ...
    def forward(
        ...
        if self.teacher_model is not None:
            pdb.set_trace()
            teacher_soft_targets = nn.functional.softmax(teacher_res.logits/self.temperature,dim=1)
            student_soft_prob = nn.functional.softmax(logits/self.temperature, dim=1)
            soft_targets_loss = torch.sum(teacher_soft_targets * (teacher_soft_targets.log() - student_soft_prob)) / student_soft_prob.size()[0] * (self.temperature**2)
