...
def main():
    ...
    if "gpt-j" in model_args.model_name_or_path:
        ...
        if ft_args.target_tokens is not None:
            ...
            if model_args.teacher_model_path != "NaN":
                teacher_model = OPTWithLMClassifier.from_pretrained(
                    model_args.teacher_model_path,
                    from_tf=bool(".ckpt" in model_args.teacher_model_path),
                    config=config,
                    cache_dir=model_args.cache_dir,
                    revision=model_args.model_revision,
                    use_auth_token=True if model_args.use_auth_token else None,
    ...
    if teacher_model is not None:
        ...
        if training_args.do_train:
            ...
            train_result = teacher_trainer.train(resume_from_checkpoint=checkpoint, ignore_keys_for_eval=["past_key_values"])
            metrics = train_result.metrics
            max_train_samples = (
                data_args.max_train_samples if data_args.max_train_samples is not None else len(
                    train_dataset)
            )
            metrics["teacher_train_samples"] = min(max_train_samples, len(train_dataset))
            teacher_trainer.log_metrics("teacher_train", metrics)
            teacher_trainer.save_metrics("teacher_train", metrics)
